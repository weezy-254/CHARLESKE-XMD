...
// You must pass in or require: botname, owner, naze, fake, my, author, packname

/**
 * Menu display function
 * @param {object} client - WhatsApp client instance (should have sendButtonMsg method)
 * @param {number|string} type - Message type
 * @param {object} m - Message object
 * @param {string} prefix - Command prefix
 * @param {string} setv - Some prefix/symbol for menu lines (e.g., '•')
 * @param {object} db - Database object with .hit and .set
 * @param {object} options - Extra options (isVip, isPremium, botNumber)
 * @param {object} config - Config object with botname, owner, naze, fake, my, author, packname
 */
async function setTemplateMenu(client, type, m, prefix, setv, db, options = {}, config = {}) {
    // Destructure config, provide fallbacks
    const {
        botname = 'BOT',
        owner = ['12345'], // array of owner numbers
        naze = { public: true },
    ...
    const day = moment.tz('Africa/Nairobi').locale('en').format('dddd');
    const date = moment.tz('Africa/Nairobi').locale('en').format('DD/MM/YYYY');
    const time = moment.tz('Africa/Nairobi').locale('en').format('HH:mm:ss');
