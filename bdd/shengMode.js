const shengMode = {
    enabled: true, // Default state is ON (even after restart)
...
const shengReplies = {
    "charles": "Yooh semaje mzee, unadai bot ama?",
...
const shengChat = async (m) => {
    ...
    if (shengMode.users[sender] === "waitingForConfirm" && paymentResponses.includes(text)) {
        m.reply("Ok poa mkuu, tuma kwa hii number 0759626063");
        delete shengMode.users[sender];
        return;
    }
    ...
    if (shengMode.users[sender] === "waitingForConfirm" && positiveResponses.includes(text)) {
        m.reply("✅ Ndo hii mkuu, pair then tuma session ID:\nhttps://charle-ke.onrender.com");
        delete shengMode.users[sender];
        return;
    }
